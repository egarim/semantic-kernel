...
namespace SemanticKernel.Connectors.UnitTests.Xpo;

/// <summary>
/// Unit tests of <see cref="XpoMemoryStore"/>.
/// </summary>
[Collection("Sequential")]
public sealed class XpoMemoryStoreTests : IDisposable
{
    private string Cnx = DevExpress.Xpo.DB.InMemoryDataStore.GetConnectionStringInMemory(true);
    private bool _disposedValue = false;

    public XpoMemoryStoreTests()
    {
    }
    ...
    {
        using XpoMemoryStore db = await XpoMemoryStore.ConnectAsync(this.Cnx);
        // Assert
        Assert.NotNull(db);
    }
